defmodule ExSyncLib.ProjectAnalyzerServer do
  @moduledoc """
  Server global GenServer that is responsible for receiving the BEAMNotify output
  """
  use GenServer
  require Logger
  alias ExSyncLib.ProjectAnalyzer.RunSrcDirs

  defstruct [:respond_to, :timeout_ref]
  alias __MODULE__, as: State

  def start_link(opts) do
    # TODO: create name using via tuple
    GenServer.start_link(__MODULE__, opts, name: __MODULE__)
  end

  def run(mixfile_path, build_path) do
    GenServer.call(__MODULE__, {:run, mixfile_path, build_path}, :infinity)
  end
  ...
  def handle_call({:run, mixfile_path, build_path}, from, state) do
    :ok = RunSrcDirs.run(mixfile_path, build_path)
